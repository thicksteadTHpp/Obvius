...
obv_internal_filter(image, x_dim, y_dim, filt, temp, x_fdim, y_fdim,
...
obv_internal_expand(image,filt,temp,x_fdim,y_fdim,xgrid_start,xgrid_step,
  ...
  for (y_pos=ygrid_start-y_fmid-1,im_pos=0; /* top */
	  ...
	  for (y_filt_lin=y_res_lin=0;
	    ...
	    for (res_pos=y_res_lin,x_filt=y_filt_lin;
		 ...
		 x_filt++,res_pos++){
	      fprintf(fp, "result[%d]: %f  image[%d]: %f  temp[%d]: %f\n",res_pos,result[res_pos], im_pos, val, x_filt, temp[x_filt]);
