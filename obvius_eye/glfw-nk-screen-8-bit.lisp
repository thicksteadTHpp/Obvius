...
(def-simple-class 8bit-GLFW-nk-screen (GLFW-nk-screen) ()  ;;[THO] removed 8bit-GL-colormap) ()
  (:default-initargs
...
(defmethod settable-parameters ((class-name (eql '8bit-GLFW-nk-screen)))
  (call-next-method))

(defmethod activate-screen :after ((screen 8bit-GLFW-nk-screen))
...
(defmethod destroy :after ((screen 8bit-GLFW-nk-screen) &key &allow-other-keys)
...
(defmethod rgb-to-float-rgb-list ((rgb cl-colors:rgb))
  (list (cl-colors:rgb-red rgb) (cl-colors:rgb-green rgb) (cl-colors:rgb-blue rgb)))

(defparameter *color-symbol-to-GL-color-map-index*
  (mapcar (lambda (c)
	    (cons (first c) (rgb-to-float-rgb-list (cdr c))))
	  `((:black   . ,cl-colors:+black+)
	    (:red     . ,cl-colors:+red+)
	    (:green   . ,cl-colors:+green+)
	    (:yellow  . ,cl-colors:+yellow+)
	    (:blue    . ,cl-colors:+blue+)
	    (:magenta . ,cl-colors:+magenta+)
	    (:cyan    . ,cl-colors:+cyan+)
	    (:white   . ,cl-colors:+white+))))


(defmethod compute-GL-color ((screen 8bit-GLFW-nk-screen) (color symbol))
  (let ((result (assoc color *color-symbol-to-GL-color-map-index*)))
    (if (null result) (rgb-to-float-rgb-list cl-colors:+white+) (cdr result))))

(defmethod compute-GL-color ((screen 8bit-GLFW-nk-screen) (RGB-list cons))
  (make-color-if-necessary screen
			   :red (first RGB-list) :green (second RGB-list) :blue (third RGB-list)))

(defmethod compute-GL-color ((screen 8bit-GLFW-nk-screen) (cindex number))
  cindex)
...
;;;
(defmethod (setf gray-shades) :around (levels (screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod (setf gray-gamma) :around (new-gamma (screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod destroy-gray-lut ((screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod set-gray-lut ((screen 8bit-GLFW-nk-screen)
			 &key
...
;;;
(defmethod print-gray-lut ((screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod (setf rgb-bits) ((RGB-list cons) (screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod (setf rgb-gamma) ((RGB-gamma cons) (screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod destroy-color-lut ((screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod set-color-lut ((screen 8bit-GLFW-nk-screen)
			  &key
...
;;;
(defmethod (setf pseudo-colors) (num (screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod (setf pseudo-color-function) (function (screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod destroy-pseudo-color-lut ((screen 8bit-GLFW-nk-screen))
...
;;;
(defmethod set-pseudo-color-lut ((screen 8bit-GLFW-nk-screen)
				 &key
...
;;;
(defmethod fill-color-table ((screen 8bit-GLFW-nk-screen) pic
...
;;;
(defmethod destroy-color-table ((screen 8bit-GLFW-nk-screen) pic)
  (with-slots (color-table) pic
    (maphash
     #'(lambda (key value)
	 (declare (ignore key))
